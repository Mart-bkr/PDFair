...
Basic visualization module for inspecting Markdown output and n-gram differences
between DeepDoctection and pdf2text extractions from WOO PDFs.
...
def visualize(self, ignore_header=True, ignore_pdf=False):
    ...
    if not ignore_pdf:
        div = html.new_tag("div")
        pdf_path = './' + self.page.doc.location.split('/')[-1]
        embed = html.new_tag("embed", src=f'{pdf_path}#page={self.page.p}', style="margin:0;min-height:100vh;width: 100%")
        div.append(embed)
        columns.append(div)
